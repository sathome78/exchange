...
jQuery(document).ready(function($) {
    ...
    $('#login_submit').on('click', function () {
        login_first = $('#auth_email').val();
        pass_first = $('#auth_pass').val();
     ...
     */
    $('#email').on("change keyup", debounce(emailValidation,400));
     ...
     */
    $('#email').on("change keyup", emailValidationInstantlyBlockSubmitButton);

    /**
     * Email validation | Recover (reset) password form
     */
    $('#email_pwd_restore').on("change keyup", emailValidationForRecoverPassword);
     ...
     * Need to instantly disabled submit button when field incorrect or invalid.
     */
    $('#password').on("change keyup", passwordValidationInstantlyBlockSubmitButton);

    /**
     * Nickname validation
     */
    $('#nickname').on("change keyup", nicknameValidation);
 ...
 * Need to instantly disabled submit button when field incorrect or invalid.
 */
function emailValidationInstantlyBlockSubmitButton(){
    var email = $(this);
    if(!email.val() || !emailPattern.test(email.val()) || !emailRegexPattern.test(email.val())) {
        $("#reg_submit").prop('disabled', true);
        return;
    }
};
 ...
 * Email validation | Recover (reset) password form
 */
function emailValidationForRecoverPassword() {
 ...
 * Need to instantly disabled submit button when field incorrect or invalid.
 */
function passwordValidationInstantlyBlockSubmitButton(){
    var password = $('#password').val();
...
function nicknameValidation(){
    var username = $(this);
    username.removeClass('field__input--error').siblings('.field__label').removeClass('field__label--error');
    $('.field__error').css('display', 'none');
    if(!username.val()) {
        username.addClass('field__input--error').siblings('.field__label').addClass('field__label--error');
        $('#nickname_required').css('display', 'block');
        $("#nickname_change__submit").prop('disabled', true);
        return;
    }
    if(nicknamePatternOnlyLatinCharacters.test(username.val())){
        username.addClass('field__input--error').siblings('.field__label').addClass('field__label--error');
        $('#nickname_only_latin_characters').css('display', 'block');
        $("#nickname_change__submit").prop('disabled', true);
        return;
    }
    if(nicknamePatternStartWithDigit.test(username.val()) || nicknamePatternStartWithSymbol.test(username.val())){
        username.addClass('field__input--error').siblings('.field__label').addClass('field__label--error');
        $('#nickname_start_with_digit_or_symbol').css('display', 'block');
        $("#nickname_change__submit").prop('disabled', true);
        return;
    }
    if(!nicknamePatternLenght.test(username.val())){
        username.addClass('field__input--error').siblings('.field__label').addClass('field__label--error');
        $('#nickname_fail_length').css('display', 'block');
        $("#nickname_change__submit").prop('disabled', true);
        return;
    }
    if(nicknamePatternContainSpecialCharacters.test(username.val())){
        username.addClass('field__input--error').siblings('.field__label').addClass('field__label--error');
        $('#nickname_contain_special_symbols').css('display', 'block');
        $("#nickname_change__submit").prop('disabled', true);
        return;
    }
    $.ajax({
        ...
        success: function(result){
            if (result.length > 0) {
                username.addClass('field__input--error').siblings('.field__label').addClass('field__label--error');
                $('#nickname_exists').css('display', 'block');
                $('#nickname_change__submit').prop('disabled', true);
            } else {
                $('#nickname_change__submit').prop('disabled', false);
                username.removeClass('field__input--error').siblings('.field__label').removeClass('field__label--error');
            }
        }
    });
...
var tryActivateRegistration = function () {
    if ((!$('#email')[0] || ($('#email').val().length != 0) && isEmailRight && $('#privacy__checked').is(":checked"))) {
...
/**
 * START
 * Geetest handler | Functions which works with catpha
 */
...
var loginHandler = function (captchaObj) {
    captchaObj.onSuccess(function () {
        ...
        if (!result) {
            ...
            $('#auth_email').val(login_first);
            $('#auth_pass').val(pass_first);
            console.log("login " + login_first);
            console.log("pass " + pass_first);
            $('#login_form').submit();
...
/**
 * Start captcha. After successful completion of the CAPTCHA, start handler.
 * @param handler
 */
