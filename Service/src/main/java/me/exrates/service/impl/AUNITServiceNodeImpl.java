...
@Service
@PropertySource({"classpath:/merchants/aunit_wallet.properties"})
public class AUNITServiceNodeImpl {
...
//  private @Value("${edc.blockchain.host_delayed}") String RPC_URL_DELAYED;
  private @Value("${url}") String url = "http://ec2-18-223-213-72.us-east-2.compute.amazonaws.com:8092";
  private String REGISTRAR_ACCOUNT = "dudotest1";
  private String REFERRER_ACCOUNT = "dudotest1";
...
//  }
  ...
  private String createAccount(final int id) throws Exception {
    ...
    final EnumMap<EDCServiceNodeImpl.KEY_TYPE, String> keys = extractKeys(makeRpcCallFast(NEW_KEY_PAIR_RPC, id)); // retrieve public and private from server
    final String response = makeRpcCallFast(REGISTER_NEW_ACCOUNT_RPC, accountName, keys.get(EDCServiceNodeImpl.KEY_TYPE.PUBLIC), keys.get(EDCServiceNodeImpl.KEY_TYPE.PUBLIC), REGISTRAR_ACCOUNT, REFERRER_ACCOUNT, String.valueOf(id));
    ...
    edcAccount.setBrainPrivKey(keys.get(EDCServiceNodeImpl.KEY_TYPE.BRAIN));
    edcAccount.setPubKey(keys.get(EDCServiceNodeImpl.KEY_TYPE.PUBLIC));
    edcAccount.setWifPrivKey(keys.get(EDCServiceNodeImpl.KEY_TYPE.PRIVATE));
...
//    edcAccountDao.create(edcAccount);
    ...
    public static void main(String[] args) throws Exception {
        AUNITServiceNodeImpl autist = new AUNITServiceNodeImpl();
        String account = autist.createAccount(1);
        System.out.println(account);
    }

  private EnumMap<EDCServiceNodeImpl.KEY_TYPE, String> extractKeys(final String json) {
    final Matcher matcher = pattern.matcher(json);
    if (matcher.find()) {
      final EnumMap<EDCServiceNodeImpl.KEY_TYPE, String> result = new EnumMap<>(EDCServiceNodeImpl.KEY_TYPE.class);
      result.put(EDCServiceNodeImpl.KEY_TYPE.BRAIN, matcher.group(1));
      result.put(EDCServiceNodeImpl.KEY_TYPE.PRIVATE, matcher.group(2));
      result.put(EDCServiceNodeImpl.KEY_TYPE.PUBLIC, matcher.group(3));
      return result;
    }
